...
# 🎚️ Seuil de filtrage
threshold = st.slider("🔎 Seuil de similarité minimum à afficher", 0.0, 1.0, 0.4, step=0.05)

# 🔘 Lancer l’analyse
if st.button("🔍 Calculer et Afficher"):

    with st.spinner(f"Encodage avec le modèle : `{model_choice}` ..."):
        similarities, doc_vecs, query_vec = calculate_similarities(model_choice, 
                                                                   query_text=query_text, 
                                                                   documents=documents)
        
        # Tri des documents par similarité décroissante
        filtered = rank_documents(documents=documents, 
                                  doc_vecs=doc_vecs, 
        ...
        if not filtered:
            ...
            fig, filtered_docs, filtered_sims = do_graph(filtered=filtered,
                                                         documents=documents,
                                                         query_vec=query_vec)
            st.pyplot(fig)
